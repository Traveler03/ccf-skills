...
本仓库是 [CCFA-Skills](https://github.com/mikubaka88/CCFA-Skills) 的同步扩展版：当前同步至上游 v0.9.0，并额外保留 `ccf-paper-reader`、新增 `ccf-paper-translator`。
...
我们因此设计了 CCFA Skills。论文不是等待逐项填充的文档，而是一条需要在反复修改中保持连贯的研究故事线。19 个分工明确的 skills 从论文阅读与全文翻译、idea、文献和实验出发，帮助论证逐渐成形，并贯穿写作、绘图、评审、rebuttal 与投稿。当任务从一个 skill 交给另一个 skill 时，研究问题、证据和结论之间的联系仍然会被保留下来。
...
| 论文精读 | `ccf-paper-reader` | 把单篇论文整理为中文优先的方法、实验与 gap 笔记 |
| 全文翻译 | `ccf-paper-translator` | 逐段忠实翻译完整论文，保留结构、公式、引用、表格和全部原图 |
...
| 精读并总结一篇论文 | `ccf-paper-reader` | 不代替完整逐段翻译 |
| 完整翻译论文并保留原图 | `ccf-paper-translator` | 不总结、改写或重绘图片 |
...
这些检查确认 19 个 skills 能被正确识别，彼此职责清楚，文档链接有效，公开文件不包含本机路径或私人信息。实验与效率结果见 [实验结果.md](实验结果.md)。
