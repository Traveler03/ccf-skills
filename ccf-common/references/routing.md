...
- One-paper summaries, selective section translation, method explanation, and experiment digestion -> `ccf-paper-reader`; a complete faithful translation preserving every section and source figure -> `ccf-paper-translator`.
...
The current runtime surface contains 19 installable `ccf-*` skills plus the LaTeX/template reference tree. Removed helper names must not be installed as standalone skills.
...
| Read and digest one paper into a Chinese-first structured note with selective translation, method/figure explanation, experiment interpretation, and gap analysis. | `ccf-paper-reader` | quick reading, standard reading, figure-aware digestion | Does not perform complete source-order translation, broad literature search, strict review, or exemplar extraction. |
| Translate one complete paper faithfully while preserving section order, equations, citations, tables, appendices, and every original source figure. | `ccf-paper-translator` | complete translation, bilingual translation, original-figure document | Does not summarize, review, rewrite, humanize, redraw figures, or invent missing source text. |
...
ccf-project-scaffolder
  ...
  -> ccf-visual-composer
  -> ccf-paper-reader / ccf-paper-translator (optional source-understanding sidecars)
...
| 精读这篇论文 / 总结 introduction / 解释方法图和实验 | `ccf-paper-reader` |
| 完整翻译这篇论文 / 逐段全文翻译 / 保留原论文所有图片 | `ccf-paper-translator` |
