...
The current customized v0.9 line has a 19-skill architecture: the upstream 17-skill family plus separate owners for one-paper reading and faithful full-paper translation. The goal is not to add a parallel lifecycle, but to keep trigger ownership clear across the existing paper-project chain.
...
| Paper understanding | `ccf-paper-reader`, `ccf-paper-translator` |
...
The table contains all 19 runtime skills. No runtime skill is currently redundant enough to delete safely.
...
| `ccf-paper-reader` vs `ccf-paper-translator` | Reader summarizes, explains, and selectively translates; translator preserves and translates the complete source in order with every original figure. | Research interpretation and faithful full-document transformation have different completeness and mutation contracts. |
| `ccf-paper-translator` vs `ccf-paper-writer` | Translator preserves author meaning and structure; writer may rewrite, polish, compress, or adapt prose. | Humanization and rewriting can change emphasis, so they must not run inside a fidelity-bound translation. |
...
Install only the 19 current runtime skills. Do not copy merged helper names into `$CODEX_HOME/skills`. If an older local install still has those helper directories, remove them before installing the current family to avoid trigger collisions. `ccf-humanization`, `ccf-literature-monitor`, `ccf-visual-composer`, `ccf-paper-reader`, `ccf-paper-translator`, and `ccf-paper-to-exemplar` are current runtime entries, not merged helper names.

## Demo Policy

The demo must recognize the current 19 runtime skills. Merged abilities still appear in the demo as modes:
