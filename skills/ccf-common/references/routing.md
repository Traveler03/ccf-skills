...
v0.8 consolidates helper skills into the owning workflow skills and adds a single-paper reading owner. Runtime surface is intentionally small: 17 installable `ccf-*` skills plus the LaTeX/template reference tree. Removed helper names must not be installed as standalone skills.
...
| Read and digest one research paper or PDF into a Chinese-first structured note. | `ccf-paper-reader` | paper reading, Introduction translation, method figure explanation, experiment summary, gap notes | Does not replace broad literature search, strict paper review/scoring, or writing exemplar extraction. |
...
ccf-project-scaffolder
  ...
  -> ccf-paper-reader (optional single-paper digestion)
...
| 读这篇论文并整理 introduction、insight、method 图和实验 | `ccf-paper-reader` |
