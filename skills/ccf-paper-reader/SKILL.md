...
- 方法或 pipeline 中间数据长什么样。若论文包含 manifest、trace report、memory entry、tool call、workflow state、evaluation record 等结构化产物，给出一个最小 JSON-style example，并在后面逐字段中文解释。示例要标明是“理解用最小示例”还是“论文原始格式”。
...
- For JSON-style examples, keep the JSON valid and compact, then provide a Chinese translation list or table for each field. Do not put Chinese comments inside the JSON block.
