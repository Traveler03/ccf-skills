...
2. Identify the intended Markdown renderer. If it is unknown, use the portable profile in `markdown-math-compatibility.md`: put whitespace before every opening inline `$` unless it begins the line; spell primes with `\prime` instead of a raw ASCII apostrophe; keep inline formula source at 40 characters or fewer; move longer, nested, or matrix/tensor-shape expressions to display blocks; use `\mathrm{name}` instead of `\operatorname{name}`; keep underscore-containing configuration keys in Markdown code or tables; avoid backslash-plus-punctuation commands that Markdown can consume; and avoid custom or HTML-producing macros.
...
6. Keep punctuation belonging to the surrounding sentence outside the math delimiter unless it is mathematically meaningful.
7. Escape literal currency dollars in prose as `\$`. Check that every `$` opens or closes an intentional math span.
8. Inside Markdown tables, avoid raw `|` in formulas because it is a column delimiter. Use `\lvert`, `\rvert`, `\mid`, or another semantically correct LaTeX command. Do not place `$$...$$` blocks inside table cells.
9. Use `aligned`, `gathered`, `cases`, `matrix`, or the source-equivalent environment for multiline structures only when the target renderer supports them. Verify braces, `\left`/`\right`, environment pairs, and row separators.
10. Run `python scripts/check_markdown_math.py <translated-file.md>` and resolve every reported error. Render-preview the completed Markdown in the intended renderer and compare formulas visually with the source PDF, including fractions, accents, matrices, cases, limits, equation numbers, and line alignment.
...
- Every opening inline `$` either begins its line or has whitespace before it; none is attached directly to punctuation, prose, parentheses, or an emphasis marker.
...
- No formula uses a backslash-plus-punctuation command such as `\%`, `\!`, `\,`, `\;`, `\{`, `\}`, or `\\` unless the actual target renderer was explicitly tested with it.
- No formula contains a raw ASCII apostrophe for a prime; use an explicit `\prime` construction.
