...
- Put whitespace before every opening inline `$` unless it begins the line. Never attach it directly to punctuation, prose, parentheses, or Markdown emphasis markers.
...
- Write primes with an explicit `\prime` construction such as `t^{\prime}`; do not use a raw ASCII apostrophe such as `t'` inside mathematics.
