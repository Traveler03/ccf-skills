...
6. Translate in source order. Keep equations and citation markers unchanged. Maintain a terminology map and use one target-language rendering for each technical term unless the source intentionally distinguishes variants.
...
10. Run the completeness and fidelity checks in `references/translation-contract.md`. Do not deliver while any source section, figure, table, footnote, appendix, or citation block is unaccounted for.
...
- Preserve equation bodies, variable names, units, signs, decimal precision, confidence intervals, and significance markers exactly.
